/**
 * GoogleMap custom component for GE lit_internal (A2UI v0.9).
 *
 * Renders an interactive Google Map with pins. Compatible with the v0.9
 * GoogleMap schema:
 *   - center: { lat, lng } | { path }
 *   - zoom:   number       | { path }
 *   - pins:   Array<{ lat, lng, name, description?, background?, borderColor?, glyphColor? }> | { path }
 *
 * v0.9 simplification: bound values are direct literals (string, number,
 * object, array) or { path } DataBindings — there are no more
 * literalString / literalNumber / literalObject / literalArray wrappers.
 *
 * To register in GE, add to lit_internal/src/v0_9/ui/custom_components/index.ts:
...
export class GoogleMap extends Root {
  ...
  accessor center: Bound<{ lat: number; lng: number }> | null = null;

  @property()
  accessor zoom: Bound<number> | null = null;

  @property({ type: Object })
  accessor pins: Bound<PinData[]> | null = null;

  @property({ type: Number })
  accessor height: number = 450;

  /** Resolve a v0.9 bound value: literal pass-through, or {path} via the data model. */
  private _resolve(val: unknown): unknown {
    ...
    if ("path" in obj && typeof obj.path === "string" && this.processor) {
      ...
      try {
        return this.processor.getData(node, obj.path, this.surfaceId ?? undefined);
      } catch {
        return null;
      }
    }

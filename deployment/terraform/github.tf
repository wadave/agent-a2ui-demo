...
# Fetch the GitHub PAT from Secret Manager to authenticate the GitHub provider
data "google_secret_manager_secret_version" "github_pat_value" {
...
provider "github" {
  ...
  token = data.google_secret_manager_secret_version.github_pat_value.secret_data
}

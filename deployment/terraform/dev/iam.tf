...
# Grant Gemini Enterprise (Discovery Engine) service account permission to invoke Cloud Run
resource "google_cloud_run_v2_service_iam_member" "gemini_enterprise_invoker" {
  project  = var.dev_project_id
  location = var.region
  name     = var.project_name
  role     = "roles/run.invoker"
  member   = "serviceAccount:service-${data.google_project.dev_project.number}@gcp-sa-discoveryengine.iam.gserviceaccount.com"

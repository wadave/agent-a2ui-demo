...
# 5. Grant Gemini Enterprise (Discovery Engine) service account permission to invoke Cloud Run
resource "google_cloud_run_v2_service_iam_member" "gemini_enterprise_invoker" {
  for_each = local.deploy_project_ids
  ...
  member   = "serviceAccount:service-${data.google_project.projects[each.key].number}@gcp-sa-discoveryengine.iam.gserviceaccount.com"

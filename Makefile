...
playground:
	...
	@echo "|                                                                             |"
	@echo "| 💡 Try asking: What is CQ performance vs budget?                             |"
	@echo "|                                                                             |"
...
deploy:
	...
	gcloud beta run deploy agent-a2ui-vega-demo \
		--source . \
		--memory "4Gi" \
		--project $$PROJECT_ID \
		--region "us-central1" \
		...
		--update-env-vars \
		"APP_URL=https://agent-a2ui-vega-demo-$$PROJECT_NUMBER.us-central1.run.app,AGENT_URL=https://agent-a2ui-vega-demo-$$PROJECT_NUMBER.us-central1.run.app,GOOGLE_CLOUD_PROJECT=$$PROJECT_ID,GOOGLE_CLOUD_LOCATION=global,GOOGLE_GENAI_USE_VERTEXAI=true,MODEL=gemini-3-flash-preview,WORKSPACE_USER_EMAIL=davewang@wangdave.altostrat.com" \
		$(if $(IAP),--iap) \
		$(if $(PORT),--port=$(PORT))
...
test-remote:
	uv sync --dev
	AGENT_URL=$(or $(AGENT_URL),https://agent-a2ui-vega-demo-$$(gcloud projects describe $$(gcloud config get-value project 2>/dev/null) --format="value(projectNumber)" 2>/dev/null).us-central1.run.app) \
...
register-gemini-enterprise:
	...
	GEMINI_DISPLAY_NAME=$${GEMINI_DISPLAY_NAME:-agent-a2ui-demo-vega} \
	uvx agent-starter-pack@0.41.1 register-gemini-enterprise \
		--agent-card-url="https://agent-a2ui-vega-demo-$$PROJECT_NUMBER.us-central1.run.app/.well-known/agent-card.json" \
